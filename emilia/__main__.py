    ...
    if update.effective_chat.type == "private":
        if len(args) >= 1:
            if args[0].lower() == "help":
                send_help(
                    update.effective_chat.id, tl(update.effective_message, HELP_STRINGS)
                )
                        ...
                        [
                            [
                                InlineKeyboardButton(
                                    text=tl(
                                        update.effective_message, "Baca di Wikipedia"
                                    ),
                                    url=pagewiki.url,
                                )
                            ]
...
def help_button(bot: Bot, update: Update):
    ...
    try:
        if mod_match:
            ...
            text = tl(
                update.effective_message, "Ini bantuan untuk modul *{}*:\n"
            ).format(HELPABLE[module].__mod_name__) + tl(
                update.effective_message, HELPABLE[module].__help__
            )
            ...
            query.message.reply_text(
                text=tl(query.message, HELP_STRINGS),
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=InlineKeyboardMarkup(
        ...
        send_help(
            ...
            InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            text=tl(update.effective_message, "Kembali"),
                            callback_data="help_back",
                        )
                    ]
                ]
    ...
    if user:
        ...
        if CHAT_SETTINGS:
            ...
            dispatcher.bot.send_message(
                user_id,
                text=tl(
                    chat_id, "Modul mana yang ingin Anda periksa untuk pengaturan {}?"
                ).format(chat_name),
                reply_markup=InlineKeyboardMarkup(
...
def donate(bot: Bot, update: Update):
    ...
    if chat.type == "private":
        update.effective_message.reply_text(
            tl(update.effective_message, DONATE_STRING),
            parse_mode=ParseMode.MARKDOWN,
