...
def join_fed(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if getfed == False:
            message.reply_text(
                tl(update.effective_message, "Silakan masukkan id federasi yang valid.")
            )
            return
...
def fed_import_bans(bot: Bot, update: Update, chat_data):
    ...
    if msg.reply_to_message and msg.reply_to_message.document:
        ...
        if fileformat == "json":
            ...
            if failed >= 1:
                tekt += tl(update.effective_message, " {} gagal di impor.").format(
                    failed
                )
            get_fedlog = sql.get_fed_log(fed_id)
            if get_fedlog:
