        ...
        if len(catatan) >= 1 and len(catatangagal) == 0:
            ...
            try:
                update.effective_message.reply_text(
                    rtext, parse_mode=ParseMode.MARKDOWN
                )
            except BadRequest:
                if conn:
                    rtext = tl(
            ...
            try:
                ...
                update.effective_message.reply_text(
                    tl(update.effective_message, rtext), parse_mode=ParseMode.HTML
                )
        else:
            if conn:
                rtext = tl(
...
def list_notes(bot: Bot, update: Update):
    ...
    if msg == tl(update.effective_message, "*Catatan di {}:*\n").format(
        chat_name
    ) or msg == tl(update.effective_message, "*Catatan lokal:*\n"):
        if conn:
            update.effective_message.reply_text(
                tl(
                    update.effective_message, "Tidak ada catatan di obrolan *{}*!"
                ).format(chat_name),
                parse_mode="markdown",
            )
        else:
            update.effective_message.reply_text(
                tl(update.effective_message, "Tidak ada catatan di obrolan ini!")
            )
    ...
    if failures:
        with BytesIO(str.encode("\n".join(failures))) as output:
            ...
            dispatcher.bot.send_document(
                ...
                caption=tl(
                    update.effective_message,
                    "File/foto ini gagal diimpor karena berasal "
...
def __stats__():
    return tl(OWNER_ID, "{} catatan, pada {} obrolan.").format(
