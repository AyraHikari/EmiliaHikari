...
def demote(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    try:
        ...
        message.reply_text(
            tl(update.effective_message, "💔 Berhasil diturunkan jabatannya!")
        )
        return (
            "<b>{}:</b>"
            "\n#DEMOTED"
    ...
    if prev_message and is_group:
        try:
            ...
            if conn:
                update.effective_message.reply_text(
                    tl(
                        update.effective_message, "Saya sudah pin pesan dalam grup {}"
                    ).format(chat_name)
                )
...
def unpin(bot: Bot, update: Update) -> str:
    ...
    try:
        ...
        if conn:
            update.effective_message.reply_text(
                tl(
                    update.effective_message, "Saya sudah unpin pesan dalam grup {}"
                ).format(chat_name)
            )
    except BadRequest as excp:
        if excp.message == "Chat_not_modified":
...
def adminlist(bot: Bot, update: Update):
    ...
    try:
        ...
        update.effective_message.reply_text(
            text, parse_mode=ParseMode.MARKDOWN, quote=False
        )
...
def permapin(bot: Bot, update: Update):
    ...
    try:
        ...
        update.effective_message.reply_text(
            tl(update.effective_message, "Saya tidak punya akses untuk pin pesan!")
        )
